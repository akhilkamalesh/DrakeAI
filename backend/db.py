...
FORBIDDEN_KEYWORDS: Set[str] = {
    ...
    "grant", "revoke", "execute", "exec", "vacuum", "copy", "commit",
    "rollback", "call", "lock", "comment", "reindex"
}


def validate_sql_safety(sql: str) -> Tuple[bool, str]:
    """
    Validates that a dynamic SQL query string is strictly a read-only SELECT/WITH
    statement referencing only permitted catalog tables, with no DDL/DML, multiple
    statements, or malicious injections.
    """
    ...
    # Reject multiple statements (semicolon followed by statements)
    ...
    if first_word not in ("SELECT", "WITH"):
        return False, f"Only SELECT or WITH queries are permitted. Got '{first_word}'."

    # Tokenize words and check forbidden keywords
    tokens = set(re.findall(r"\b[a-zA-Z_]+\b", cleaned.lower()))
    disallowed = tokens.intersection(FORBIDDEN_KEYWORDS)
    if disallowed:
        return False, f"Query contains forbidden SQL commands: {disallowed}."

    # Extract CTE names if present (e.g., WITH cte_name AS (...))
    cte_names = set(re.findall(r"\b([a-zA-Z0-9_]+)\s+as\s*\(", cleaned, flags=re.IGNORECASE))
    cte_names_lower = {c.lower() for c in cte_names}

    # Extract tables after FROM and JOIN
    table_matches = re.findall(r"\b(?:from|join)\s+([a-zA-Z0-9_]+)", cleaned, flags=re.IGNORECASE)
    for tbl in table_matches:
        tbl_lower = tbl.lower()
        if tbl_lower not in ALLOWED_TABLES and tbl_lower not in cte_names_lower:
            return False, f"Unauthorized table or subquery alias '{tbl}'. Permitted tables: {ALLOWED_TABLES}."
...
def search_context_graph(
    ...
    and audio features. Supports dynamic SQL queries generated by the Text-to-SQL Agent
    (with strict safety validation and fallback) or parameterized standard context graph search.
    ...
    # Validate custom dynamic SQL if provided
    active_sql = CONTEXT_GRAPH_SQL
    active_params = query_params
    used_custom_sql = False

    if custom_sql:
        is_safe, safety_msg = validate_sql_safety(custom_sql)
        if is_safe:
            active_sql = custom_sql
            active_params = dict(query_params)
            if custom_params:
                active_params.update(custom_params)
            active_params["query_vector"] = query_vector
            active_params["excluded_track_ids"] = clean_excluded
            active_params["fetch_limit"] = fetch_limit
            active_params["limit"] = limit
            used_custom_sql = True
        else:
            logger.warning("Dynamic SQL failed safety validation: %s. Using default context graph SQL.", safety_msg)
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                try:
                    ...
                    logger.warning("Execution of SQL failed: %s. Falling back to default context graph SQL.", e)
                    cur.execute(CONTEXT_GRAPH_SQL, query_params)
                    rows = cur.fetchall()

                # If dynamic SQL returned 0 rows, fall back to default context graph
                if len(rows) == 0 and used_custom_sql:
                    logger.info("Dynamic SQL yielded 0 rows. Falling back to standard context graph search.")
                    cur.execute(CONTEXT_GRAPH_SQL, query_params)
                    rows = cur.fetchall()
        ...
        logger.warning("Database unavailable in search_context_graph: %s", e)
        return []
